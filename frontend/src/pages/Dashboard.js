...
const Profile = ({ user, data, prod }) => {
  const [imageSrc, setImageSrc] = useState(
    `${process.env.REACT_APP_API}/${user.profilePicUrl}`
  );
  const handleImageError = () => {
    console.log('error occured', imageSrc);
    setImageSrc(
      'https://filmfare.wwmindia.com/thumb/content/2016/May/paresh_1464614885.jpg?width=1200&height=900'
    );
    console.log(user);
  };
  console.log(user);
                ...
                <div className="flex flex-wrap justify-center">
                  ...
                  <div className="w-full lg:w-4/12 px-4 lg:order-1">
                    <div className="flex justify-center py-4 lg:pt-4 pt-8">
                      ...
                      <div className="mr-4 p-3 text-center">
                        <span className="text-xl font-bold block uppercase tracking-wide text-blueGray-600">
                          {user.type === 'Seller' ? prod.length : null}
                        </span>
                        <span className="text-sm text-blueGray-400">
                          {user.type === 'Seller' ? 'Products' : null}
                        </span>
                      </div>
                    </div>
                  </div>
...
const Sellerproducts = ({ prod, handleDelete }) => {
  console.log(prod);
  return (
    <div className="flex flex-col h-[100vh] overflow-y-scroll hide-scrollbar w-full p-4 bg-blueGray-200">
          ...
          <li className="pb-3 sm:pb-4">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-[var(--clr-primary-5)] truncate ">
                ₹{prod.price}
                </p>
              </div>
              <div className="inline-flex items-center text-[var(--clr-primary-5)] text-base font-semibold">
                {prod.stock ? prod.stock:0}
              </div>
              <button
                className="text-[var(--clr-primary-5)]"
                onClick={() => handleDelete(prod._id)}
              >
                <FaTrash className='text-[var(--clr-primary-5)] text-lg'/>
              </button>
            </div>
          </li>
        ))}
      ...
      <Support>
        <main>
          <section>
            ...
            <p>
              To create an account, click the "Sign Up" button at the top of the
              page and follow the prompts to enter your information.
            </p>
            <h3>How do I reset my password?</h3>
            <p>
              To reset your password, click the "Forgot Password" link on the
              login page and follow the prompts to reset your password.
            </p>
...
const DasSideBar = ({ handleItemClick, user }) => {
  ...
  return (
    <aside
      ...
      className={
        isSidebarOpen
          ? 'hidden z-40 w-20 md:60 h-screen transition-transform '
          : 'relative z-40 w-20 md:w-64 h-screen'
      }
      aria-label="Sidebar"
    >
      <div className="h-full flex px-3 py-4 flex-col align-center bg-gray-50 ">
        <a href="/" className="flex items-center pl-2.5 mb-5">
          <img src={logo} className="h-6 mr-3 hidden md:block" alt="Logo" />
        </a>
        <ul className="space-y-2 font-medium">
          ...
          {user.type === 'Seller' && (
            <li>
              <div
                onClick={() => handleItemClick('sellerorders')}
                className="flex items-center p-2 text-gray-900 rounded-lg hover:bg-[var(--clr-primary-2)]  group"
              >
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (user.type === 'Seller') {
      axios
        .get(`${process.env.REACT_APP_API}/api/orders/sell/${user.email}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
      ...
      axios
        .get(`${process.env.REACT_APP_API}/api/products/seller/${user.email}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
  ...
  const handleStatusChange = (oid, pid, newStatus) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API}/api/orders/${oid}/items/${pid}/status/`,
        {
          status: newStatus,
        },
        {
  ...
  return (
    <div className="flex flex-row">
      ...
      {selectedItem === 'orders' && <Orders data={data} />}
      ...
      {selectedItem === 'support' && <SupportPage />}
    </div>
  );
}
